...
const Tab = styled.button.withConfig({
  ...
  &:hover {
    background: ${props => (props.active ? theme.colors.primaryDark : theme.colors.gray100)};
  }
`;
...
const FooterSectionEdit = () => {
  ...
  useEffect(() => {
    if (section) {
      setSectionData({
        ...
        contactEmail: section.content?.contactEmail || vendor?.businessInfo?.email || '',
        contactPhone: section.content?.contactPhone || vendor?.businessInfo?.phone || '',
        address: section.content?.address || vendor?.businessInfo?.address || '',
        socialLinks: section.content?.socialLinks || [],
        footerLinks: section.content?.footerLinks || [],
        copyrightText: section.content?.copyrightText || `© ${new Date().getFullYear()} ${vendor?.name || 'Your Store'}. All rights reserved.`
      });
    }
  }, [section, vendor]);
  ...
  const addSocialLink = () => {
    const newSocialLinks = [...sectionData.socialLinks, { platform: 'facebook', url: '', icon: 'FaFacebook' }];
  ...
  const removeSocialLink = (index) => {
    const newSocialLinks = sectionData.socialLinks.filter((_, i) => i !== index);
    handleInputChange('socialLinks', newSocialLinks);
  };
  ...
  const removeFooterLink = (index) => {
    const newFooterLinks = sectionData.footerLinks.filter((_, i) => i !== index);
    handleInputChange('footerLinks', newFooterLinks);
  };
