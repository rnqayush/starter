...
import { getBusinessWebsiteData, updateBusinessWebsiteData } from '../DummyData/businessWebsiteData';
...
export const fetchBusinessData = async (businessSlug) => {
  try {
    ...
    console.log(`[FAKE API] Successfully fetched data for: ${businessSlug}`, businessData);
    
    return createAPIResponse(businessData, true, 'Business data fetched successfully');
...
export const updateBusinessData = async (businessSlug, updatedData) => {
  try {
    console.log(`[FAKE API] Updating business data for: ${businessSlug}`, updatedData);
    
    // Simulate network delay
    await simulateDelay(400);
    
    const updatedBusiness = updateBusinessWebsiteData(businessSlug, updatedData);
    
    if (!updatedBusiness) {
      throw new Error(`Failed to update business with slug "${businessSlug}"`);
    }
    
    console.log(`[FAKE API] Successfully updated data for: ${businessSlug}`, updatedBusiness);
    
    return createAPIResponse(updatedBusiness, true, 'Business data updated successfully');
 ...
 */
export const fetchBusinessSection = async (businessSlug, sectionName = null) => {
  try {
    console.log(`[FAKE API] Fetching section "${sectionName}" for business: ${businessSlug}`);
    ...
    if (sectionName && !sectionData) {
      throw new Error(`Section "${sectionName}" not found for business "${businessSlug}"`);
    }
    
    console.log(`[FAKE API] Successfully fetched section data:`, sectionData);
    
    return createAPIResponse(sectionData, true, `Section data fetched successfully`);
 ...
 */
export const updateBusinessSection = async (businessSlug, sectionName, sectionData) => {
  try {
    console.log(`[FAKE API] Updating section "${sectionName}" for business: ${businessSlug}`, sectionData);
    ...
    if (!updatedBusiness) {
      throw new Error(`Failed to update section "${sectionName}" for business "${businessSlug}"`);
    }
    
    console.log(`[FAKE API] Successfully updated section "${sectionName}":`, updatedBusiness[sectionName]);
    
    return createAPIResponse(updatedBusiness[sectionName], true, `Section "${sectionName}" updated successfully`);
...
export const fetchMultipleBusinessData = async (businessSlugs) => {
  try {
    ...
    console.log(`[FAKE API] Successfully fetched multiple business data:`, businessesData);
    
    return createAPIResponse(businessesData, true, 'Multiple business data fetched successfully');
